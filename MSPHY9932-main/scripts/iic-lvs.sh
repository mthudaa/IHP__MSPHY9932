...
if [ "$VERILOG_MODE" -eq 0 ]; then
	...
	# SPICE netlists for the standard cells (runs for both xschem and user-provided netlists)
	STDCELL_GREP="${STD_CELL_PREFIX:-$STD_CELL_LIBRARY}"
	if grep -q "$STDCELL_GREP" "$NETLIST_SCH"; then
			# Remove the .end
			sed -i '/\.end\b/d' "$NETLIST_SCH"
			# Append stdcell library
			STDCELL_FILE="$PDK_ROOT/$PDK/libs.ref/$STD_CELL_LIBRARY/spice/$STD_CELL_LIBRARY.spice"
			if [ -f "$STDCELL_FILE" ]; then
				cat "$STDCELL_FILE" >> "$NETLIST_SCH"
			else
				echo "[WARNING] Standard cell SPICE file not found: $STDCELL_FILE"
			fi
			# Add .end
...
# fix top-level port order in extracted netlist to match schematic
# ----------------------------------------------------------------
if [ "$VERILOG_MODE" -eq 0 ]; then
	# extract port order from schematic netlist
	SCH_PORTS=$(head -1 "$NETLIST_SCH" | grep "^\.subckt $TOPCELL " | sed "s/^\.subckt $TOPCELL //")
	LAY_PORTS=$(head -1 "$NETLIST_LAY" | grep "^\.subckt $TOPCELL " | sed "s/^\.subckt $TOPCELL //")
	if [ -n "$SCH_PORTS" ] && [ -n "$LAY_PORTS" ] && [ "$SCH_PORTS" != "$LAY_PORTS" ]; then
		echo "[INFO] Fixing port order in extracted netlist to match schematic..."
		sed -i "s/^\.subckt $TOPCELL .*/\.subckt $TOPCELL $SCH_PORTS/" "$NETLIST_LAY"
	fi
fi

# flatten device wrappers into M-element MOSFETs (IHP PDK)
# ---------------------------------------------------------
# Magic extracts nmos/pmos GDS cells as wrapper subcircuits, then instantiates
# them as X-elements. Netgen cannot match these against schematic M-elements.
# This step removes the wrappers and converts instances to proper M-elements.
# Also handles the case where Magic emits X-elements without wrappers.
FLATTEN_SCRIPT="$(dirname "$0")/flatten_mos_devices.py"
if echo "$PDK" | grep -q -i "ihp-sg13" && [ -f "$FLATTEN_SCRIPT" ]; then
	echo "[INFO] Flattening MOS device wrappers in $NETLIST_LAY..."
	python3 "$FLATTEN_SCRIPT" "$NETLIST_LAY"
fi

# create custom LVS setup
# ----------------------
LVS_SETUP="$PDK_ROOT/$PDK/libs.tech/netgen/${PDK}_setup.tcl"
...
# Check LVS result: accept if flattened netlists match uniquely
# (pin matching may fail for symmetric circuits, which is a soft pass)
if grep -i -q "Netlists do not match" "$LVS_REPORT"; then
	echo "LVS ERRORS FOUND! Please check <$LVS_REPORT>!" 
	echo "---"
	exit $ERR_LVS_MISMATCH
elif grep -i -q "Netlists match uniquely" "$LVS_REPORT"; then
